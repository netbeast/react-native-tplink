...
const getHostIPs = require('./utils').getHostIPs
...
class Client extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.socket.on('message', async (msg, rinfo) => {
      ...
      if(await getHostIPs() === rinfo.address) return
      ...
      if (this.devices.has(sysinfo.deviceId)) {
        ...
        plug.power = sysinfo.relay_state === 1;
        plug.onTime = sysinfo.on_time;
  ...
  sendDiscovery (devices) {
    ...
    const msgBuf = encrypt(commands.search);
